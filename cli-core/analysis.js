...
async function analyseURL(browser, pageInformations, options) {
  ...
  try {
    ...
    let pages = await startActions(
      ...
      TIMEOUT,
      pptrHar,
      pageInformations.pageName ? pageInformations.pageName : 'Lancement du scénario'
    );
    ...
    console.error(
      `Error while analyzing URL ${pageInformations.url} : `,
      error,
    );
...
async function waitPageLoading(page, pageInformations, TIMEOUT) {
  if (pageInformations.waitForSelector) {
    await page.waitForSelector(pageInformations.waitForSelector, {
      visible: true,
      timeout: TIMEOUT,
    });
  } else if (pageInformations.waitForXPath) {
    await page.waitForXPath(pageInformations.waitForXPath, {
      visible: true,
      timeout: TIMEOUT,
    });
    ...
    for (let index = 0; index < actions.length; index++) {
      ...
      try {
        if (action.pageName) {
          ...
          currentPage = {};
          currentPage.name = action.pageName;
          results = [];
          nbRequest = 0;
...
async function doAnalysis(page, pptrHar, name) {
  ...
  try {
    ...
    await page.evaluate(
      (x) => (resources = x),
      ressourceTree.frameTree.resources,
    );
